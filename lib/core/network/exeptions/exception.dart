...
class ServerException implements Exception {
  ...
  factory ServerException.defaultError() {
    return ServerException(
      ErrorMessageModel(
        statusMessage: "Un expected Error Happened",
        statusCode: 550,
      ),
    );
  }

  factory ServerException.withMessage(String message) {
    return ServerException(
      ErrorMessageModel(
...
/// This type of Exception [ApplicationUnderMaintenanceException] is thrown if the application's server is under repair or
/// if there are any errors related to the temporary cessation of server side services.
class ApplicationUnderMaintenanceException implements Exception {
  final String message;
  ApplicationUnderMaintenanceException(
      {this.message = 'exception: Maintenance Mode'});
}
