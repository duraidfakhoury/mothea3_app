...
class CacheService {
  ...
  bool get hasRefreshToken  =>  _preferences?.containsKey(_Keys.refreshToken) ?? false ;

  Future<void> removeRefreshToken () async => await _preferences?.remove(_Keys.refreshToken);

  String get refreshToken => _preferences?.getString(_Keys.refreshToken) ?? "no refresh token";
  ...
  Future<void> removeLanguage() async => await _preferences?.remove(_Keys.language);
